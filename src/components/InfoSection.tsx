...
const InfoSection = () => {
  ...
  return <section id="info" className="py-16 md:py-24 bg-secondary/30">
      <div className="container px-6 md:px-12">
        <div className="mb-12 text-center max-w-3xl mx-auto">
          <h2 className={cn("text-3xl md:text-4xl font-bold mb-4 transition-all duration-700", isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4")}>
            Understanding <span className="text-primary">AI Intelligence</span>
          </h2>
          <p className={cn("text-lg text-muted-foreground transition-all duration-700 delay-100", isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4")}>
            Discover how AI agents and automation technologies are transforming industries and creating new possibilities
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          {infoCards.map((card, index) => <Card key={index} className={cn("border border-border bg-card hover:shadow-md transition-all duration-300", "transform transition-all duration-700", isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4")} style={{
        ...
        <div className={cn("text-center transition-all duration-700 delay-700", isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4")}>
